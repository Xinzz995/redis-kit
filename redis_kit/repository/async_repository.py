...
class AsyncRepository:
    ...
    async def save(self, entity: T) -> T:
        ...
        if not entity.id:
            ...
            allowed = await self._lock_script(keys=[key], args=[str(entity.version)])
            if not allowed:
        ...
        key = self._make_key(entity.id)
        await self._client.hset(key, mapping=self._to_hash(entity))
    ...
    async def delete(self, entity_id: str) -> None:
        ...
        now = datetime.now()
        await self._client.hset(key, mapping={"deleted": "1", "deleted_at": now.isoformat()})
